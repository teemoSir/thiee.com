...
function addtraffic(RType) {
    if (_ty_cluster) {
        ...
        clearInterval(_tracffic_t);
    }

    var baidu_source;
    if (RType == 1) {
        baidu_source = new ol.source.XYZ({
            url: "http://map.so.com/app/traffic.php?act=tile&x={x}&y={-y}&z={z}"
        });
    } else if (RType == 2) {
        baidu_source = new ol.source.XYZ({
          //  url: "http://rtt2b.map.qq.com/rtt/?z={z}&x={x}&y={y}"
    ...
    _map.addLayer(baidu_layer);
    _ty_cluster.push(baidu_layer);
}
